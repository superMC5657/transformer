...
class Transformer(nn.Module, ABC):
    def __init__(self, n_src_vocab, n_trg_vocab, src_pad_idx, trg_pad_idx, d_word_vec=512, d_model=512,
                 d_inner=2048,
                 n_layers=6, n_head=8, d_k=64, d_v=64, dropout=0.1, n_position=200, trg_emb_prj_weight_sharing=True,
                 src_emb_prj_weight_sharing=True):
        super().__init__()
        ...
        if trg_emb_prj_weight_sharing:
            # Share the weight between trg word embedding & last dense layer
            self.trg_word_prj.weight = self.decoder.trg_word_emb.weight
            self.x_logit_scale = (d_model ** -0.5)
        if src_emb_prj_weight_sharing:
